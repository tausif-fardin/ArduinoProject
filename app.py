...
def get_data():
    try:
        ...
        # Write the voltage and current values to the xlsx file
        wb = load_workbook(filename='data.xlsx')
        ws = wb['Sheet1']
        max_row = ws.max_row
        ws.cell(row=max_row+1, column=1, value=voltage)
        ws.cell(row=max_row+1, column=2, value=current)
        wb.save('data.xlsx')
                      
        # Return the voltage and current values as JSON
