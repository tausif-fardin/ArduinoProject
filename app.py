    ...
    try:
        ...
        if line != '':
            ...
            if len(splitted) == 9:
                ...
                voltages = [float(splitted[i]) for i in range(1, 5)]
                currents = [float(splitted[i]) for i in range(5, 9)]
        
        # Write the temperature, voltage and current values to the excel file
        max_row = ws.max_row
        ws.append([temperature] + voltages + currents)
        wb.save('data.xlsx')
        ...
        print('Temperature:', temperature, 'Voltages:', voltages, 'Currents:', currents)
